...
-- Migration 2026-09-15: le Magasin.
--
-- WHY. A festival asks its benevoles « peux-tu ramener du matos ? » and runs a store of equipment,
-- some owned, some lent. The regisseur asked for a mode to manage the general equipment: available,
-- lent, given back.
--
-- WHAT CHANGES.
--   event.equipment, a ledger [{key, name, quantity, lender, lenderKind, lenderKey, status, holder,
--   note}], empty for every event already there.
--   volunteer.equipment_note, the form's answer as typed, new and empty.
--   load_plan and write_plan_body replaced whole, identical to migration 31's but for those.
--
-- APPLY AFTER 31. min_plan_format GOES TO 24.
--
...
alter table event     add column if not exists equipment jsonb not null default '[]'::jsonb;
alter table volunteer add column if not exists equipment_note text not null default '';

do $chk$
begin
  if not exists (select 1 from pg_constraint where conname = 'event_equipment_check') then
    alter table event add constraint event_equipment_check check (jsonb_typeof(equipment) = 'array');
  end if;
end;
$chk$;
...
update app_setting set number = 24 where name = 'min_plan_format';
