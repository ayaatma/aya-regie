-- migration-sequence: 18
-- Migration 2026-09-13: la billetterie, et les invitations des artistes par leur nom.
--
-- WHY. The people at the entrance need one list of everybody who gets in without a ticket:
-- benevoles, orgas, artistes, their guests, prestataires, other invitations, each with the
-- drink and meal tickets to hand them, the kind of entry it is worth and the bracelet. The
-- list is DERIVED (tools/src/ticketing.ts); what the base gains is the configuration behind it
-- and the few decisions the regisseur takes by hand. See .claude/memory/feature_ticketing.md.
--
-- WHAT CHANGES.
--   event.guests_per_artist: how many named guests each member of an act may bring.
--   ticket_type: a kind of entry and the stretch of the event it opens.
--   bracelet_type + bracelet_default: a wristband and the statuses it goes to by default.
--   extra_person: somebody the door alone knows (prestataire, autre invitation).
--   ticketing_choice: a ticket or a bracelet picked against the default, per person.
--   artist_guest: an act's invitations BY NAME. It replaces artist_member.invited (a tick that
--   named nobody) and artist.extra_invitations (a count): the count is converted into that many
--   unnamed guests of the act, so nothing typed today is lost, and the two columns are dropped.
--
-- THE TWO FUNCTIONS ARE REPLACED WHOLE, as always.
--
-- min_plan_format GOES TO 10. A browser tab built before today writes every act back without
-- its guests and the plan back with no billetterie at all.
--
-- Idempotent: re-running it changes nothing. The conversion of the counts is guarded by the
-- presence of the column it reads, so it runs exactly once.

-- ---------------------------------------------------------------------------
-- 1. The new columns and tables
-- ---------------------------------------------------------------------------

alter table event add column if not exists guests_per_artist int not null default 1;

do $constraints$
begin
  if not exists (select 1 from pg_constraint where conname = 'event_guests_per_artist_check') then
    alter table event add constraint event_guests_per_artist_check check (guests_per_artist >= 0);
  end if;
end
$constraints$;

create table if not exists artist_guest (
...
create index if not exists artist_guest_artist_idx on artist_guest (artist_id);
create index if not exists artist_guest_member_idx on artist_guest (member_id);
...
create index if not exists ticket_type_event_idx on ticket_type (event_id);

create table if not exists bracelet_type (
  id         uuid primary key default gen_random_uuid(),
  event_id   uuid not null references event(id) on delete cascade,
  key        text not null,
  label      text not null default '',
...
create index if not exists bracelet_type_event_idx on bracelet_type (event_id);

create table if not exists bracelet_default (
...
create table if not exists extra_person (
  id            uuid primary key default gen_random_uuid(),
  event_id      uuid not null references event(id) on delete cascade,
  key           text not null,
  first_name    text not null default '',
...
create index if not exists extra_person_event_idx on extra_person (event_id);

create table if not exists ticketing_choice (
  id             uuid primary key default gen_random_uuid(),
  event_id       uuid not null references event(id) on delete cascade,
...
create index if not exists ticketing_choice_event_idx on ticketing_choice (event_id);

-- ---------------------------------------------------------------------------
-- 2. The data: the counted invitations become unnamed guests, then the two columns go
-- ---------------------------------------------------------------------------

do $data$
begin
  if exists (select 1 from information_schema.columns
             where table_name = 'artist' and column_name = 'extra_invitations') then
    insert into artist_guest (artist_id, member_id, key, first_name, last_name, sort_order)
    select a.id, null, 'invite-' || n, '', 'Invité ' || n, n - 1
    from artist a
    cross join lateral generate_series(1, a.extra_invitations) as s(n)
    where a.extra_invitations > 0
      and not exists (select 1 from artist_guest g where g.artist_id = a.id and g.key = 'invite-' || n);

    alter table artist drop column extra_invitations;
  end if;

  if exists (select 1 from information_schema.columns
             where table_name = 'artist_member' and column_name = 'invited') then
    alter table artist_member drop column invited;
  end if;
end
$data$;
...
declare t text;
begin
  foreach t in array array['artist_guest', 'ticket_type', 'bracelet_type', 'bracelet_default',
                           'extra_person', 'ticketing_choice']
  loop
...
as $fn$
  ...
  from event e, slots, preference_slots, poles, shifts, artists, organisers, leader_roles, volunteers, buddies,
       assignments, reserve, organiser_shifts, catering, ticketing
...
begin
  ...
  insert into extra_person (event_id, key, first_name, last_name, status, phone, note,
                            drink_tickets, meal_tickets, sort_order)
  select p_event_id, x->>'key', coalesce(x->>'firstName', ''), coalesce(x->>'lastName', ''),
         coalesce(x->>'status', 'autre'), coalesce(x->>'phone', ''), coalesce(x->>'note', ''),
  ...
  -- choice left with neither is not written, which is what "the default" means.
  insert into ticketing_choice (event_id, person_kind, person_key, ticket_type_id, bracelet_id,
                                sort_order)
  select p_event_id, x->>'personKind', x->>'personKey', ct.id, cb.id, (ord - 1)::int
  ...
  where num_nonnulls(ct.id, cb.id) >= 1
  on conflict do nothing;
...
update app_setting set number = 10 where name = 'min_plan_format';
