-- migration-sequence: 20
-- Migration 2026-09-13: les Réglages avancés, par événement.
--
-- WHY. The régisseur asked for the tool to serve any event, and for each event to decide which
-- rule of the planning blocks and which one only costs, with its weight. Until today that answer
-- was written in the code for the Loto Tekno: nine rules always blocked and seventeen weights were
-- constants. `Plan.constraints` carries the event's own answer.
--
-- WHAT CHANGES.
--   event.constraint_settings jsonb, '{}' by default, which is every criterion at its default:
--   exactly the rules and weights every existing event was planned under, so nothing moves.
--   load_plan returns it as 'constraints'; write_plan_body stores it, or '{}' when absent.
--
-- WHY A DOCUMENT AND NOT COLUMNS: see the comment on the column in db/schema.sql.
--
-- min_plan_format GOES TO 12. A tab built before today writes the plan back without its
-- constraints, which is every rule the régisseur loosened put back to blocking without a word.
--
...
alter table event add column if not exists constraint_settings jsonb not null default '{}'::jsonb;

do $constraints$
begin
  if not exists (select 1 from pg_constraint where conname = 'event_constraint_settings_check') then
    alter table event add constraint event_constraint_settings_check
...
update app_setting set number = 12 where name = 'min_plan_format';
