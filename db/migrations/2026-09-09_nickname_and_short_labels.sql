...
-- Migration 2026-09-09: the surname question is read, and a short label stops being ambiguous.
--
-- WHY, in two parts, both of them about what a name means on a screen.
--
-- 1. A grid box is about eleven characters wide, so the grid, a volunteer's own schedule and the
--    public planning have always written "Marie D." rather than "Marie Dubois". That is not a
--    name when there are two of them: two Marie whose surnames both start with a D read as one
--    person, and a régisseur reassigning "Marie D." at two in the morning has no way to tell
--    which one they just moved. The surname is now cut at the shortest prefix that tells the
--    person apart from everybody else sharing their given name, volunteers and pole leaders
--    counted together, and only the people who actually collide grow.
--
-- 2. The form asks "Surnom (si tu préfères qu'on t'appelle par celui-ci)" and the answer was
--    thrown away. It was one of the three decoy columns the binder had to be taught to refuse,
--    and refusing it was only ever half the job: what somebody's team calls them on the night is
--    exactly what should be on the box. When it is filled in it replaces the first name in every
--    short label, and nowhere else. The welcome desk's list, the exports and anything the
--    association has to check a person against keep the registered name.
--
-- WHERE THE LABEL IS COMPUTED, and why it is stored. How much of a surname a label needs is a
-- question about the whole roster. get_volunteer_schedule and get_public_planning below answer
-- one person at a time, so computing it here would mean a second implementation of
-- tools/src/display.ts that has to agree with the first one forever. The browser computes it and
-- every save rewrites all of them, so display_name is a cache recomputed in full on each write,
-- never a fact anybody edits.
--
-- MUST SHIP WITH ITS DEPLOY. It raises min_plan_format to 3, so a browser built before today is
-- refused and told to reload rather than saving a document with no surname in it.
--
...
alter table volunteer add column if not exists nickname     text not null default '';
alter table volunteer add column if not exists display_name text not null default '';

comment on column volunteer.nickname is
  'Surnom (si tu préfères qu''on t''appelle par celui-ci), tel que répondu. Vide sinon.';
comment on column volunteer.display_name is
  'Le nom court affiché partout où le nom entier ne tient pas. Calculé par le navigateur sur '
  'l''ensemble des personnes, réécrit à chaque enregistrement.';

-- Existing rows get the form this database produced on its own until today: readable, and not a
-- step backwards from what the volunteer view showed an hour ago. The first save from the tool
-- replaces every one of them with the real label.
update volunteer
   set display_name = btrim(first_name ||
                            case when last_name = '' then '' else ' ' || left(last_name, 1) || '.' end)
 where display_name = '';

-- ---------------------------------------------------------------------------
-- 2. load_plan hands the surname to the browser, which is what makes it survive a save.
--
-- display_name is deliberately NOT in what goes out: the browser recomputes it from the whole
-- roster and sends it back on the next save, so returning it would be handing back a cache the
-- receiver is about to rebuild anyway.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. write_plan_body stores both, with a fallback for a document that carries no label.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. The two anonymous read paths stop assembling a name and read the column.
-- ---------------------------------------------------------------------------
...
--
-- Volunteer gains a field, so a browser built before today writes volunteers with no surname in
-- them. Letting it save would wipe the answer for everybody who gave one, in one write, without
-- a word.
-- ---------------------------------------------------------------------------

update app_setting set number = 3 where name = 'min_plan_format';

-- The stored rows carry the new shape from the moment the columns above exist.
update event set plan_format = 3 where plan_format < 3;

-- ---------------------------------------------------------------------------
-- The ledger row, written inside the same transaction as everything above it.
-- ---------------------------------------------------------------------------

insert into schema_migration (sequence, filename, note) values
  (10, '2026-09-09_nickname_and_short_labels.sql',
       'Le surnom du formulaire est lu, et le nom court distingue deux homonymes')
on conflict (sequence) do update
  set applied_at = now(), note = excluded.note;
