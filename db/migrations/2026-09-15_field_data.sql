-- migration-sequence: 28
-- Migration 2026-09-15: les donnees de terrain.
--
-- WHY. A festival form collects what is needed in a hurry: who to call in an emergency, health
-- problems or specific needs, a birth date. The regisseur asked for them on the fiche, visible to
-- the regie and the responsables only.
--
-- WHAT CHANGES.
--   volunteer.emergency_contact, health_note, minor, nickname_matters; organiser.emergency_contact,
--   health_note. All new, empty or null. The birth date is read at import into « minor » and never
--   stored.
--   load_plan and write_plan_body replaced whole, identical to migration 27's but for those.
--   get_organiser_planning replaced: an orga holding no pole reads the plan without the emergency
--   contacts and health notes of anybody. Every orga used to read the whole plan, and still does
--   for everything else.
--
-- APPLY AFTER 27. min_plan_format GOES TO 20.
--
...
alter table volunteer add column if not exists emergency_contact text not null default '';
alter table volunteer add column if not exists health_note text not null default '';
alter table volunteer add column if not exists minor boolean;
alter table volunteer add column if not exists nickname_matters boolean;
alter table organiser add column if not exists emergency_contact text not null default '';
alter table organiser add column if not exists health_note text not null default '';

-- ---------------------------------------------------------------------------
-- 2. What an orga reads, field data stripped for an orga holding no pole
-- ---------------------------------------------------------------------------
...
update app_setting set number = 20 where name = 'min_plan_format';
