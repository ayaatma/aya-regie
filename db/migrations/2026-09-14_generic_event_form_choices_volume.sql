-- migration-sequence: 21
-- Migration 2026-09-14: l'outil cesse de supposer le formulaire du Loto Tekno.
--
-- WHY. Four requests of the regisseur, one round: any number of pole choices, ranked or not; a
-- volume per day on events longer than a day, with the hour a day hands over to the next; the
-- import's columns and answers remembered per event; binômes added or removed by hand.
--
-- WHAT CHANGES.
--   volunteer_choice, new: the choices in rank order. Filled from choice1_* and choice2_*, which
--     are then dropped. A choice with neither a pole nor an answer is not carried: an unanswered
--     second choice was stored as nothing, and a row would count a choice nobody made.
--   volunteer.requested_hours: int to numeric(6,2).
--   event: pole_choices_ranked, volume_scope, day_start_hour, volume_options, form_mapping.
--   buddy_pair: manual, dismissed.
--   load_plan and write_plan_body replaced whole.
--
-- REQUIRES MIGRATION 20 (event.constraint_settings), which the functions below read.
--
-- min_plan_format GOES TO 13. A tab built before today writes every volunteer back with no
-- choice at all.
--
-- Idempotent: re-running it changes nothing. The data is carried only while the old columns exist.

-- ---------------------------------------------------------------------------
-- 1. The new table and columns
-- ---------------------------------------------------------------------------

create table if not exists volunteer_choice (
...
do $policy$
begin
  if not exists (select 1 from pg_policies where tablename = 'volunteer_choice'
                 and policyname = 'volunteer_choice_organiser') then
    create policy volunteer_choice_organiser on volunteer_choice
      for all to authenticated using (true) with check (true);
  end if;
end
$policy$;

alter table event add column if not exists pole_choices_ranked boolean not null default true;
alter table event add column if not exists volume_scope text not null default 'event';
alter table event add column if not exists day_start_hour numeric(4,2) not null default 12;
alter table event add column if not exists volume_options numeric(6,2)[] not null default '{4,6,8}';
alter table event add column if not exists form_mapping jsonb not null default '{}'::jsonb;

alter table buddy_pair add column if not exists manual boolean not null default false;
alter table buddy_pair add column if not exists dismissed boolean not null default false;

do $constraints$
begin
  if not exists (select 1 from pg_constraint where conname = 'event_volume_scope_check') then
    alter table event add constraint event_volume_scope_check check (volume_scope in ('event', 'day'));
  end if;
  if not exists (select 1 from pg_constraint where conname = 'event_day_start_hour_check') then
    alter table event add constraint event_day_start_hour_check
      check (day_start_hour >= 0 and day_start_hour < 24);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'event_form_mapping_check') then
    alter table event add constraint event_form_mapping_check check (jsonb_typeof(form_mapping) = 'object');
  end if;
end
$constraints$;

alter table volunteer alter column requested_hours type numeric(6,2);

-- ---------------------------------------------------------------------------
-- 2. The data: the two flat choices become rows, then the columns go
-- ---------------------------------------------------------------------------

do $data$
begin
  if exists (select 1 from information_schema.columns
             where table_name = 'volunteer' and column_name = 'choice1_pole_id') then
    insert into volunteer_choice (volunteer_id, rank, pole_id, raw, level)
    select v.id, 0, v.choice1_pole_id, v.choice1_raw, coalesce(v.choice1_level, 'debutant')
    from volunteer v
    where v.choice1_pole_id is not null or v.choice1_raw <> ''
    on conflict do nothing;

    insert into volunteer_choice (volunteer_id, rank, pole_id, raw, level)
    select v.id,
           case when v.choice1_pole_id is not null or v.choice1_raw <> '' then 1 else 0 end,
           v.choice2_pole_id, v.choice2_raw, coalesce(v.choice2_level, 'debutant')
    from volunteer v
    where v.choice2_pole_id is not null or v.choice2_raw <> ''
    on conflict do nothing;

    -- A correction of one choice is a correction of the list now.
    update volunteer
    set manual_fields = (
      select coalesce(array_agg(distinct f), '{}'::text[])
      from unnest(array_replace(array_replace(array_replace(array_replace(
             manual_fields, 'choice1PoleKey', 'choices'), 'choice1Level', 'choices'),
             'choice2PoleKey', 'choices'), 'choice2Level', 'choices')) as f)
    where manual_fields && array['choice1PoleKey', 'choice1Level', 'choice2PoleKey', 'choice2Level'];

    alter table volunteer drop column choice1_pole_id;
    alter table volunteer drop column choice1_raw;
    alter table volunteer drop column choice1_level;
    alter table volunteer drop column choice2_pole_id;
    alter table volunteer drop column choice2_raw;
    alter table volunteer drop column choice2_level;
  end if;
end
$data$;
...
-- ---------------------------------------------------------------------------
-- 4. The guard against a stale tab
-- ---------------------------------------------------------------------------

update app_setting set number = 13 where name = 'min_plan_format';
