-- migration-sequence: 25
-- Migration 2026-09-15: les tranches a eviter.
--
-- WHY. A festival form asks « Peux-tu faire des shifts de nuit ? » with three answers: oui, « oui
-- mais je prefere ne pas en faire si possible », non. The tool knew a refused tranche (a rule) and
-- a preferred one (a score), not a tranche somebody would rather avoid without refusing it.
--
-- WHAT CHANGES.
--   volunteer.avoided_slot_keys, new, empty for everybody already there.
--   load_plan and write_plan_body replaced whole, identical to migration 24's but for that column.
--   The criterion itself (avoidedSlot, weight 1000 an hour) lives in event.constraint_settings,
--   which already stores only what a regisseur changed: nothing to add for it here.
--
-- APPLY AFTER 24. min_plan_format GOES TO 17.
--
...
alter table volunteer add column if not exists avoided_slot_keys text[] not null default '{}';
...
update app_setting set number = 17 where name = 'min_plan_format';
