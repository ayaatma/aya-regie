-- migration-sequence: 23
-- Migration 2026-09-15: la reserve dans la liste d'entree, un reglage par evenement.
--
-- WHY. The reserve left the grids' pool pane and became a list of its own on the Personnes tab,
-- because a benevole in reserve does zero hours and is normally not on site. Whether they are on
-- the door's export is the regisseur's call per event (a reserve benevole may still come to the
-- montage), so it is a setting and not a rule.
--
-- WHAT CHANGES.
--   event.reserve_on_door_list, new, false for every event already there.
--   load_plan and write_plan_body replaced whole, identical to migration 22's but for that column.
--
-- min_plan_format GOES TO 15. A tab built before today writes the setting back as off.
--
...
alter table event add column if not exists reserve_on_door_list boolean not null default false;
...
update app_setting set number = 15 where name = 'min_plan_format';
