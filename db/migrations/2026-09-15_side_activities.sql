-- migration-sequence: 31
-- Migration 2026-09-15: les activites annexes.
--
-- WHY. A festival asks who would come to a pre-montage the week before, to weekends preparing the
-- site, to weekends of deco. The regisseur wants the list of people keen on each, without a grid.
--
-- WHAT CHANGES.
--   event.side_activities, [{key, label, when}], empty for every event already there.
--   volunteer.side_activity_keys and organiser.side_activity_keys, new and empty.
--   load_plan and write_plan_body replaced whole, identical to migration 30's but for those.
--
-- APPLY AFTER 30. min_plan_format GOES TO 23.
--
...
alter table event     add column if not exists side_activities jsonb not null default '[]'::jsonb;
alter table volunteer add column if not exists side_activity_keys text[] not null default '{}';
alter table organiser add column if not exists side_activity_keys text[] not null default '{}';

do $chk$
begin
  if not exists (select 1 from pg_constraint where conname = 'event_side_activities_check') then
    alter table event add constraint event_side_activities_check check (jsonb_typeof(side_activities) = 'array');
  end if;
end;
$chk$;
...
update app_setting set number = 23 where name = 'min_plan_format';
