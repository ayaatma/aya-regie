-- migration-sequence: 22
-- Migration 2026-09-15: une personne saisie à la main.
--
-- WHY. The Billetterie tab became « Personnes », where the regisseur can turn an orga into a
-- benevole. That benevole has no row in the form's export, so the next import would list them among
-- the absents, ticked for removal like somebody who withdrew. volunteer.entered_by_hand says the
-- person never came from the form, and the import screen keeps them by default.
--
-- WHAT CHANGES.
--   volunteer.entered_by_hand, new, false for everybody already there.
--   load_plan and write_plan_body replaced whole, identical to migration 21's but for that column.
--
-- min_plan_format GOES TO 14. A tab built before today writes such a benevole back as if they came
-- from the form.
--
...
alter table volunteer add column if not exists entered_by_hand boolean not null default false;
...
update app_setting set number = 14 where name = 'min_plan_format';