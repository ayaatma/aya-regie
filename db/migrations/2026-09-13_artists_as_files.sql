-- migration-sequence: 17
-- Migration 2026-09-13: les artistes deviennent des fiches.
--
-- WHY. artist was a name and the two hours of its set, which is all the validator ever reads
-- (a benevole placed across a set they asked not to miss). The regisseur asked for everything
-- else they know about an act and had nowhere to write: how many they are, who they are, what
-- each of them eats and is handed, how they are paid, how they travel and what it costs, when
-- they do their balances, how long the changement de plateau takes on each side of the set, and
-- what they need on stage. See .claude/memory/feature_artists.md.
--
-- WHAT CHANGES.
--   artist gains the columns of the fiche. Every one of them has a default, and the default is
--   the state an act nobody has filled in is in: one person, no changeover, nothing to book, no
--   balances. The balances are stored in hours from the EVENT's start and may be negative: a
--   soundcheck the afternoon before the doors open is drawn on the montage, translated at read
--   time, and stored nowhere else.
--   artist_member is one person of an act, keyed under the act. drink_tickets null means
--   "follow event.artist_drinks", which is the new figure on the event: the regisseur asked that
--   changing the setting reach every member, and the way to do that is not to copy it into them.
--   artist_car_trip is one journey the association reimburses.
--   artist_member.volunteer_id / organiser_id say who a member ALSO is: one person, one row on
--   the caterer's sheet, and event.artist_drinks_cumulative says whether their tickets add up.
--   meal_choice gains artist_member_id: a member's plates are ticked like an orga's, computed
--   from the act's hours on the venue and stored only where the regisseur disagreed.
--
-- NOTHING ANYBODY TYPED IS LOST. The four columns that existed keep their values; the new ones
-- take their defaults on every existing row.
--
-- THE TWO FUNCTIONS ARE REPLACED WHOLE, as always: db-check compares tables and columns, never
-- function bodies.
--
-- min_plan_format GOES TO 9. A browser tab built before today writes every act back as a name
-- and two hours, which is every member, every trajet and every ticked plate gone at once.
--
...
alter table event add column if not exists artist_drinks int not null default 2;
alter table event add column if not exists artist_drinks_cumulative boolean not null default false;

do $constraints$
begin
  if not exists (select 1 from pg_constraint where conname = 'event_artist_drinks_check') then
    alter table event add constraint event_artist_drinks_check check (artist_drinks >= 0);
  end if;
end
$constraints$;

comment on column event.artist_drinks is
  'Tickets boisson remis a chaque membre d''un groupe, quelles que soient ses heures. Depuis le 2026-09-13.';

alter table artist add column if not exists size                int          not null default 1;
alter table artist add column if not exists changeover_before   numeric(6,2) not null default 0;
alter table artist add column if not exists changeover_after    numeric(6,2) not null default 0;
alter table artist add column if not exists train_tickets       int          not null default 0;
alter table artist add column if not exists train_done          boolean      not null default false;
alter table artist add column if not exists plane_tickets       int          not null default 0;
alter table artist add column if not exists plane_done          boolean      not null default false;
alter table artist add column if not exists technical_needs     text         not null default '';
alter table artist add column if not exists patch_size          int          not null default 0;
alter table artist add column if not exists notes               text         not null default '';
alter table artist add column if not exists soundcheck_needed   boolean      not null default false;
alter table artist add column if not exists soundcheck_start    numeric(6,2) not null default 0;
alter table artist add column if not exists soundcheck_end      numeric(6,2) not null default 0;
alter table artist add column if not exists soundcheck_engineer boolean      not null default false;
alter table artist add column if not exists extra_invitations   int          not null default 0;

do $constraints$
begin
  if not exists (select 1 from pg_constraint where conname = 'artist_size_check') then
    alter table artist add constraint artist_size_check check (size >= 0);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'artist_changeover_before_check') then
    alter table artist add constraint artist_changeover_before_check check (changeover_before >= 0);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'artist_changeover_after_check') then
    alter table artist add constraint artist_changeover_after_check check (changeover_after >= 0);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'artist_train_tickets_check') then
    alter table artist add constraint artist_train_tickets_check check (train_tickets >= 0);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'artist_plane_tickets_check') then
    alter table artist add constraint artist_plane_tickets_check check (plane_tickets >= 0);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'artist_patch_size_check') then
    alter table artist add constraint artist_patch_size_check check (patch_size >= 0);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'artist_extra_invitations_check') then
    alter table artist add constraint artist_extra_invitations_check check (extra_invitations >= 0);
  end if;
end
$constraints$;

comment on column artist.soundcheck_start is
  'Debut des balances, en heures depuis le debut de l''evenement. Negatif quand elles tombent la veille, pendant le montage.';
...
create table if not exists artist_member (
  ...
  -- The benevole or the orga this member ALSO is: at most one, null for nobody, set null when
  -- the person goes so the member is simply counted on their own again.
...
create index if not exists artist_member_artist_idx on artist_member (artist_id);

create table if not exists artist_car_trip (
  id                  uuid primary key default gen_random_uuid(),
  artist_id           uuid not null references artist(id) on delete cascade,
  key                 text not null,
...
create index if not exists artist_car_trip_artist_idx on artist_car_trip (artist_id);

-- meal_choice names one of THREE people now. The old check is dropped by the name Postgres gave
-- it and the new one added under a name of our own, so a re-run finds it and stops.
alter table meal_choice add column if not exists artist_member_id uuid references artist_member(id) on delete cascade;

do $check$
begin
  if exists (select 1 from pg_constraint where conname = 'meal_choice_check') then
    alter table meal_choice drop constraint meal_choice_check;
  end if;
  if not exists (select 1 from pg_constraint where conname = 'meal_choice_one_person_check') then
    alter table meal_choice add constraint meal_choice_one_person_check
      check (num_nonnulls(volunteer_id, organiser_id, artist_member_id) = 1);
  end if;
end
$check$;

drop index if exists meal_choice_person_idx;
create index if not exists meal_choice_person_idx
  on meal_choice (volunteer_id, organiser_id, artist_member_id);
...
declare t text;
begin
  foreach t in array array['artist_member', 'artist_car_trip']
  loop
...
as $fn$
  ...
  artists as (
    select coalesce(jsonb_agg(jsonb_build_object(
             ...
             'extraInvitations',   a.extra_invitations,
             'members', coalesce((
...
begin
  ...
  insert into artist (event_id, key, name, start_hours, end_hours, sort_order,
                      ...
                      train_tickets, train_done, plane_tickets, plane_done,
                      technical_needs, patch_size, notes,
  ...
  select p_event_id, x->>'key', x->>'name',
         ...
         coalesce((x->>'extraInvitations')::int, 0)
  ...
  insert into artist_member (artist_id, key, first_name, last_name, role, diet, allergies,
                             drink_tickets, payment, invited, volunteer_id, organiser_id,
                             sort_order)
  select a.id, m->>'key',
         ...
         coalesce((m->>'invited')::boolean, true),
         lv.id, lo.id,
         (ord - 1)::int
  ...
  insert into artist_car_trip (artist_id, key, from_address, from_venue, to_address, to_venue,
                               fuel, consumption_per_100, tolls, sort_order)
  select a.id, c->>'key',
...
update app_setting set number = 9 where name = 'min_plan_format';
