...
-- Migration 2026-09-10: two questions of the form become free text, and a fiche becomes
-- correctable by hand.
--
-- WHY. The form used to ask "which slot can you absolutely not work" as a radio button and
-- "which pole" as a list. It now asks the first as a sentence, and offers an "Autre" box
-- beside each pole choice. Both answers therefore have to be INTERPRETED, and an
-- interpretation can be wrong: "je ne peux pas avant 18h" is one slot or two depending on the
-- night, and "la buvette du fond" is a pole nobody listed.
--
-- SO THREE THINGS CHANGE HERE, and they only make sense together.
--
-- 1. THE ANSWER IS KEPT AS TYPED. availability_note, choice1_raw and choice2_raw hold the
--    sentence, forever, beside whatever the parser made of it. A regisseur correcting a
--    reading has to be able to read the original, and no screen in the tool may rewrite it.
--
-- 2. THE REFUSED SLOT BECOMES A LIST. volunteer_refused_slot replaces refused_slot_key, for
--    exactly the reason volunteer_refused_pole replaced refused_pole_id on 2026-09-08: a
--    sentence can name two slots, this is a hard constraint, and a refusal dropped in silence
--    is somebody placed at an hour they told us they could not come. Every existing answer
--    becomes a list of one, which is the same answer.
--
-- 3. A FICHE CARRIES ITS OWN DOUBT AND ITS OWN CORRECTIONS. needs_review and review_reasons
--    are set by the importer whenever it had to guess; manual_fields records every answer a
--    regisseur corrected, and is what stops the next export from quietly undoing the
--    correction. Both are the tool talking about the fiche, never answers.
--
-- MUST SHIP WITH ITS DEPLOY. It raises min_plan_format to 4, so a browser built before today
-- is refused and told to reload rather than saving a document with a single refused slot in
-- it, which would silently drop the other refusals of everybody who named two.
--
-- Everything here is idempotent except the drop of refused_slot_key, which is guarded.

-- ---------------------------------------------------------------------------
-- 1. The answers as typed, and the fiche bookkeeping
-- ---------------------------------------------------------------------------

alter table volunteer add column if not exists availability_note text    not null default '';
alter table volunteer add column if not exists choice1_raw       text    not null default '';
alter table volunteer add column if not exists choice2_raw       text    not null default '';
alter table volunteer add column if not exists manual_fields     text[]  not null default '{}';
alter table volunteer add column if not exists needs_review      boolean not null default false;
alter table volunteer add column if not exists review_reasons    text[]  not null default '{}';

comment on column volunteer.availability_note is
  'La contrainte horaire dans les mots du bénévole, telle que saisie. Les tranches refusées en sont une interprétation, jamais l''inverse.';
comment on column volunteer.manual_fields is
  'Les réponses corrigées à la main par le régisseur. Un ré-import ne les écrase pas.';
comment on column volunteer.needs_review is
  'Vrai tant qu''un humain n''a pas relu une interprétation dont l''import n''était pas sûr.';

-- ---------------------------------------------------------------------------
-- 2. The refused slots, as a list
-- ---------------------------------------------------------------------------

create table if not exists volunteer_refused_slot (
  volunteer_id uuid not null references volunteer(id) on delete cascade,
  slot_key     text not null,
  primary key (volunteer_id, slot_key)
);

-- The backfill, and it runs before the drop for the obvious reason. Every answer given under
-- the old question becomes a list of one: the same answer, said in the new shape.
do $$
begin
  if exists (select 1 from information_schema.columns
             where table_schema = 'public' and table_name = 'volunteer'
               and column_name = 'refused_slot_key') then
    insert into volunteer_refused_slot (volunteer_id, slot_key)
    select id, refused_slot_key from volunteer
    where refused_slot_key is not null and refused_slot_key <> ''
    on conflict do nothing;

    alter table volunteer drop column refused_slot_key;
  end if;
end $$;

-- ---------------------------------------------------------------------------
-- 3. The two functions that carry a plan in and out, replaced whole
--
-- Copied from db/schema.sql, which is the mirror this migration keeps true. They are replaced
-- rather than patched because a function has no ALTER that adds a column to its output.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. The guard against a stale browser tab
--
-- A tab loaded before this deploy writes plans with `refusedSlot` and no note. Saving one
-- would drop the second refusal of everybody who named two, and erase every sentence the
-- volunteers typed. It is refused and told to reload.
-- ---------------------------------------------------------------------------

update app_setting set number = 4 where name = 'min_plan_format';
