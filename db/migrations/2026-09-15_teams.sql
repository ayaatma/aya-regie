-- migration-sequence: 30
-- Migration 2026-09-15: les equipes.
--
-- WHY. A festival keeps groups of four to six benevoles together as one crew on a pole's roulement.
-- The regisseur asked for a per-event « fonctionnement en equipe », never blocking, with the
-- composition of each team visible and a team readable on the grid.
--
-- WHAT CHANGES.
--   event.teams_enabled (false) and event.teams ([{key, name, poleKey}], empty), new.
--   volunteer.team_key, new, null for everybody already there.
--   load_plan and write_plan_body replaced whole, identical to migration 29's but for those.
--   The criterion (teamSplit, weight 400 an hour) lives in event.constraint_settings.
--
-- APPLY AFTER 29. min_plan_format GOES TO 22.
--
...
alter table event     add column if not exists teams_enabled boolean not null default false;
alter table event     add column if not exists teams jsonb not null default '[]'::jsonb;
alter table volunteer add column if not exists team_key text;

do $chk$
begin
  if not exists (select 1 from pg_constraint where conname = 'event_teams_check') then
    alter table event add constraint event_teams_check check (jsonb_typeof(teams) = 'array');
  end if;
end;
$chk$;
...
update app_setting set number = 22 where name = 'min_plan_format';
