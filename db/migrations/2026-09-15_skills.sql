-- migration-sequence: 27
-- Migration 2026-09-15: les competences.
--
-- WHY. A festival form calls its competence question « TRES IMPORTANTE »: permis, CACES, metier,
-- bricolage; and its montage teams include « conducteurices d'engins », a qualification across
-- teams rather than a team. The regisseur asked for competence tags on people and poles that ask
-- for them, on the exploit and on the two phases.
--
-- WHAT CHANGES.
--   event.skills, [{key, label}], empty for every event already there.
--   volunteer.skills, volunteer.skills_note, organiser.skills, pole.required_skills,
--   phase_pole.required_skills, all new and empty.
--   load_plan and write_plan_body replaced whole, identical to migration 26's but for those.
--   The criterion (missingSkill, weight 4000 an hour) lives in event.constraint_settings.
--
-- APPLY AFTER 26. min_plan_format GOES TO 19.
--
...
alter table event      add column if not exists skills jsonb not null default '[]'::jsonb;
alter table volunteer  add column if not exists skills text[] not null default '{}';
alter table volunteer  add column if not exists skills_note text not null default '';
alter table organiser  add column if not exists skills text[] not null default '{}';
alter table pole       add column if not exists required_skills text[] not null default '{}';
alter table phase_pole add column if not exists required_skills text[] not null default '{}';

do $chk$
begin
  if not exists (select 1 from pg_constraint where conname = 'event_skills_check') then
    alter table event add constraint event_skills_check check (jsonb_typeof(skills) = 'array');
  end if;
end;
$chk$;
...
update app_setting set number = 19 where name = 'min_plan_format';
