-- migration-sequence: 19
-- Migration 2026-09-13: le prix des trajets, le téléphone référent, et la billetterie éditable.
--
-- WHY. Five remarks after the billetterie went live. The door edits a person's drink tickets and
-- leaves a remark on anybody, not only on the extras; an act records what its train and plane
-- tickets cost and a number to call; a car journey records its distance and cost, which
-- « Calculer automatiquement » fills from the map and the event's own fuel and toll rates.
--
-- WHY A MIGRATION OF ITS OWN. These columns were first added to migration 18 after it had
-- already been applied, which is the one thing a migration must never undergo: 18 is back to
-- what ran, and everything new is here.
--
-- WHAT CHANGES.
--   event: fuel_price_essence / diesel / electrique / gpl / autre, toll_per_km.
--   artist: train_cost, plane_cost, contact_phone.
--   artist_car_trip: distance_km, cost (null until computed or typed).
--   ticketing_choice: drink_tickets (null = the computed figure), note.
--   extra_person.note is dropped, its text carried over to that person's ticketing_choice.note:
--   the remark now lives in one place for everybody.
--
-- min_plan_format GOES TO 11. A tab built before today writes every choice back without its
-- figure or remark and the plan without its travel rates.
--
-- Idempotent: re-running it changes nothing. The note is carried over only while the column exists.

-- ---------------------------------------------------------------------------
-- 1. The new columns
-- ---------------------------------------------------------------------------

alter table event add column if not exists fuel_price_essence    numeric(6,3) not null default 1.75;
alter table event add column if not exists fuel_price_diesel     numeric(6,3) not null default 1.70;
alter table event add column if not exists fuel_price_electrique numeric(6,3) not null default 0.22;
alter table event add column if not exists fuel_price_gpl        numeric(6,3) not null default 0.95;
alter table event add column if not exists fuel_price_autre      numeric(6,3) not null default 1.75;
alter table event add column if not exists toll_per_km           numeric(6,3) not null default 0.10;

alter table artist add column if not exists train_cost    numeric(9,2) not null default 0;
alter table artist add column if not exists plane_cost    numeric(9,2) not null default 0;
alter table artist add column if not exists contact_phone text not null default '';

alter table artist_car_trip add column if not exists distance_km numeric(8,1);
alter table artist_car_trip add column if not exists cost        numeric(9,2);

alter table ticketing_choice add column if not exists drink_tickets int;
alter table ticketing_choice add column if not exists note text not null default '';

do $constraints$
begin
  if not exists (select 1 from pg_constraint where conname = 'ticketing_choice_drink_tickets_check') then
    alter table ticketing_choice add constraint ticketing_choice_drink_tickets_check
      check (drink_tickets is null or drink_tickets >= 0);
  end if;
end
$constraints$;

-- ---------------------------------------------------------------------------
-- 2. The data: an extra person's note becomes their remark, then the column goes
-- ---------------------------------------------------------------------------

do $data$
begin
  if exists (select 1 from information_schema.columns
             where table_name = 'extra_person' and column_name = 'note') then
    update ticketing_choice c set note = x.note
    from extra_person x
    where c.event_id = x.event_id and c.person_kind = 'extra' and c.person_key = x.key
      and c.note = '' and x.note <> '';

    insert into ticketing_choice (event_id, person_kind, person_key, note)
    select x.event_id, 'extra', x.key, x.note
    from extra_person x
    where x.note <> ''
      and not exists (select 1 from ticketing_choice c
                      where c.event_id = x.event_id and c.person_kind = 'extra' and c.person_key = x.key);

    alter table extra_person drop column note;
  end if;
end
$data$;
...
as $fn$
  ...
  from event e, slots, preference_slots, poles, shifts, artists, organisers, leader_roles, volunteers, buddies,
       assignments, reserve, organiser_shifts, catering, ticketing, travel
...
update app_setting set number = 11 where name = 'min_plan_format';
