-- migration-sequence: 26
-- Migration 2026-09-15: les disponibilites jour par jour.
--
-- WHY. A festival over several days asks when somebody arrives on the first day and leaves on the
-- last, and ticks the montage days they come. The regisseur asked for availability held day by day
-- in the tool. The phase days already fit volunteer_phase_window; the exploit needed a place for
-- an arrival, a departure and a day off.
--
-- WHAT CHANGES.
--   volunteer.unavailable, new, [{start, end}] in event hours, empty for everybody already there.
--   load_plan and write_plan_body replaced whole, identical to migration 25's but for that column.
--
-- APPLY AFTER 25. min_plan_format GOES TO 18.
--
...
alter table volunteer add column if not exists unavailable jsonb not null default '[]'::jsonb;
...
update app_setting set number = 18 where name = 'min_plan_format';
