-- migration-sequence: 29
-- Migration 2026-09-15: le pole impose par un·e responsable.
--
-- WHY. On a festival, a responsable brings people for a precise post (« envoye·e par un respo sur
-- un poste precis »). The regisseur asked for that placement to be made by default, while keeping
-- the right to change everything.
--
-- WHAT CHANGES.
--   volunteer.imposed_pole_key, new, null for everybody already there.
--   load_plan and write_plan_body replaced whole, identical to migration 28's but for that column.
--   The criterion (imposedPole, weight 10000 an hour) lives in event.constraint_settings.
--
-- APPLY AFTER 28. min_plan_format GOES TO 21.
--
...
alter table volunteer add column if not exists imposed_pole_key text;
...
update app_setting set number = 21 where name = 'min_plan_format';
