...
-- Migration 2026-09-17: la duree par defaut d'un creneau, au niveau de l'evenement.
--
-- WHY. The regisseur wants one default length for the event, which every pole follows unless it
-- was set by hand for that pole. Until now each pole carried its own copy, always.
--
-- WHAT CHANGES.
--   event.default_shift_hours, numeric, 2 for every event already there.
--   pole.default_shift_hours now means « set by hand »: null follows the event. Every pole holding
--   exactly 2, the value every pole got when nobody chose, is set back to null so it follows the
--   event (whose value is 2, so nothing a creneau would get changes). A pole at any other length
--   keeps it, as a choice.
--   load_plan and write_plan_body replaced whole, identical to migration 32's but for the event's
--   value.
--
-- APPLY AFTER 32. min_plan_format GOES TO 25.
--
-- Idempotent: re-running it changes nothing (a pole set by hand to 2 since would follow the event
-- again, which with an event at 2 draws the same créneaux).

-- ---------------------------------------------------------------------------
-- 1. The column, and the poles that were never given a length
-- ---------------------------------------------------------------------------

alter table event add column if not exists default_shift_hours numeric not null default 2;

do $chk$
begin
  if not exists (select 1 from pg_constraint where conname = 'event_default_shift_hours_check') then
    alter table event add constraint event_default_shift_hours_check check (default_shift_hours > 0);
  end if;
end;
$chk$;

update pole p set default_shift_hours = null
from event e
where p.event_id = e.id
  and p.default_shift_hours is not null
  and p.default_shift_hours = e.default_shift_hours;
...
as $fn$
  ...
  volunteers as (
    select coalesce(jsonb_agg(jsonb_build_object(
             ...
             'equipmentNote',   v.equipment_note,
...
begin
  ...
  insert into volunteer (event_id, key, first_name, last_name, nickname, display_name,
                         ...
                         imposed_pole_key, team_key, side_activity_keys, equipment_note,
...
update app_setting set number = 25 where name = 'min_plan_format';
